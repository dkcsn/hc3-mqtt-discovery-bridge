...
6. Press **Request Discovery** if publishers listen for the HA Birth topic.
7. Verify that children appear and update.
...
Scenes and other QuickApps can call `getEntity(externalId)`, `getEntities()`, `getStatus()`, `requestDiscovery()`, `reconnect()`, `publish(topic, payload, retain, qos)` and `sendCommand(externalId, action, value)`. Returned registry data is copied so callers cannot mutate live state accidentally.

## Icon

`assets/mqtt-discovery-source.png` preserves the supplied 1254 × 1254 source. `assets/mqtt-discovery-128.png` is the verified 128 × 128 HC3 upload asset. Uploading an icon to HC3 yields a numeric ID; `IconRegistry.lua` reads that stable assignment from the `IconQaRegistry` global variable. The QuickApp never embeds or guesses an HC3 icon ID.
...
See [architecture](docs/ARCHITECTURE.md), [discovery semantics](docs/MQTT_DISCOVERY.md) and the [compatibility matrix](docs/COMPATIBILITY.md).
