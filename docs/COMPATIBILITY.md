...
Status as of version 0.3.2. `FULL` means the intended basic state/command model is implemented, not every optional Home Assistant attribute.
...
| `light` | PARTIAL | multilevel switch | on/off, JSON brightness or separate brightness topic | on, off, separate/scalar brightness | RGB/RGBW, color temperature and effects remain metadata only. |
...
| `device_tracker` | FULL | binary sensor | configurable home/not-home payloads | — | Exposes presence as a binary value. |
| `siren` | BASIC | binary switch | on/off | on, off | Tone, duration and volume features remain metadata only. |
| `fan` | BASIC | multilevel switch | on/off and percentage | on, off, percentage | Presets, oscillation and direction remain metadata only. |
| `lock`, `climate`, `text`, `event`, `vacuum`, `valve` | PARSE-ONLY | none | Metadata retained | none | Adapter not implemented. |
...
| Schema 1/2 registry migration | YES, automatic |
| Large registry chunking/integrity | YES |
